...
### 📄 .gitignore
The `.gitignore` file tells Git which files and folders to leave untracked. This keeps the repository clean and prevents sensitive or unnecessary files from being pushed. It covers:

- **Virtual environments** (`venv/`, `.venv/`) — local Python environments that shouldn't be shared
- **Environment variables** (`.env`) — API keys and secrets that must stay private
- **Python cache** (`__pycache__/`, `*.pyc`) — auto-generated bytecode files
- **Streamlit secrets** (`.streamlit/secrets.toml`) — credentials used by Streamlit Cloud
- **Data files** (`*.csv`, `*.xlsx`) — large or raw data files better managed outside Git
- **Model artifacts** (`*.pkl`, `*.joblib`) — trained model files that can be large or regenerated
- **OS & editor clutter** (`.DS_Store`, `.vscode/`, `.idea/`) — system and IDE-specific files
